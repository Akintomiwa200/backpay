...
export default function PinVerificationPage() {
  ...
  // const { isSuccess, isPending, mutate, data: mutationData } = usePayMutation();
  const {
    isSuccess,
    isPending,
    mutate,
  ...
  console.log({ data, error, isError, mutationData });
    ...
    <div className="flex justify-center gap-4">
      {pinArray.map((digit, index) => (
        <input
          ...
          autoFocus={index === 0}
          // @ts-ignore
          disabled={isError && error?.status === 404}
        />
      ))}
    </div>
  );
  ...
  useEffect(() => {
    ...
    if (mutationData?.data?.success || isCancelSucess) {
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <div className="flex flex-col items-center animate-fade-in">
          ...
          <div className="text-center mb-6">
            {data?.data?.type?.toLowerCase() === 'data' && (
              <p className="text-primary-700 text-sm">
                {data?.data?.selectedBundle?.title}
              </p>
            )}
            ...
            <div className="space-y-1">
              <p className="text-text-secondary text-base">
                ...
                <span className="font-medium text-text-primary">
                  {data?.data?.meterNumber || data?.data?.recipient}
                </span>
            ...
            <div className="space-y-4">
              ...
              <div className="text-center pt-4 border-t border-border-light">
                <button
                  ...
                  className="text-sm font-medium text-text-tertiary hover:text-error-600 transition-colors duration-200"
                >
                  Cancel Transaction
                </button>
